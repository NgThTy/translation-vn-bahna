...
def evaluation_cli_args(config: Dict[str, Any]) -> List[str]:
    args = [
        ...
        "--loss_type", str(config["loss_type"]),
        "--retrieval", str(config["retrieval"]),
        "--neighborhood_k", str(config["neighborhood_k"]),
        "--src_max_len", str(config["src_max_len"]),
        "--tgt_max_len", str(config["tgt_max_len"]),
        "--batch_size", str(config["batch_size"]),
        "--eval_batch_size", str(max(1, int(config["batch_size"]))),
...
def evaluate_checkpoint_on_dev(
    ...
    if args.eval_retrievals == "all":
        retrievals = list(VALID_RETRIEVALS)
    elif args.eval_retrievals == "both":
        retrievals = ["cosine", "csls"]
    else:
        retrievals = [args.eval_retrievals]
    ...
    for retrieval in retrievals:
        ...
        result_dir = Path(args.dev_output_root) / configuration_name
        metrics, ranks, topk_idx, scores = evaluate_embeddings(
            src_emb, tgt_emb, retrieval, args.topk_eval, neighborhood_k,
            [int(value) for value in args.eval_ks],
        )
        config = build_configuration(
            metadata, checkpoint_dir, retrieval, neighborhood_k
        )
        write_result(
            args, result_dir, "dev", configuration_name, config, args.dev_csv,
            df, raw_src, raw_tgt, norm_src, norm_tgt,
...
def evaluate_mode(args: argparse.Namespace) -> None:
    ...
    if neighborhood_k < 1:
        raise ValueError("--neighborhood_k must be positive")
    config = build_configuration(
        metadata, checkpoint_dir, args.retrieval, neighborhood_k
    )
    if args.split_name == "test":
        authorize_test(Path(args.selection_manifest), args.configuration_name, config)
    ...
    metrics, ranks, topk_idx, scores = evaluate_embeddings(
        src_emb, tgt_emb, args.retrieval, args.topk_eval, neighborhood_k,
        [int(value) for value in args.eval_ks],
    )
    write_result(
        args, Path(args.output_dir), args.split_name, args.configuration_name,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--eval_retrievals",
        choices=["cosine", "csls", "margin_ratio", "both", "all"],
        default="all",
        help=(
            "Development criteria to evaluate. Use all for cosine, CSLS, and "
            "ratio margin; both is retained as the legacy cosine+CSLS option."
        ),
    )
    parser.add_argument(
        "--retrieval",
        ...
        default="cosine",
        help="Retrieval criterion for single-checkpoint evaluation mode",
    )
    parser.add_argument(
        "--neighborhood_k",
        type=int,
