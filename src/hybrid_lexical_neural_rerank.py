...
The IBM1 and XLM-R components must themselves already be selected on the
training-derived development split. The selected XLM-R component may use
cosine, CSLS, or Artetxe-Schwenk ratio-margin retrieval; the hybrid consumes
that exact development-selected criterion for both candidate generation and
full-pool neural scoring. Hybrid stage order, candidate top-k, normalization,
formula, interpolation weights, and length penalty are tuned on ``dev.csv``.
Exactly one selected hybrid configuration may access ``test.csv``.
"""
...
try:
    from retrieval_scoring import VALID_RETRIEVALS, retrieval_score_matrix
except ImportError:  # Supports module-style execution from the project root.
    from src.retrieval_scoring import VALID_RETRIEVALS, retrieval_score_matrix
...
def neural_retrieval_settings(selected_neural: Dict[str, Any]) -> Tuple[str, int]:
    """Return the selected neural criterion and its full-pool neighborhood size.

    ``neighborhood_k`` is the reviewer-compliant field used by the updated
    evaluators. ``csls_k`` is accepted only as a backward-compatible fallback
    for older cosine/CSLS development artifacts.
    """
    configuration = selected_neural["configuration"]
    retrieval = str(
        configuration.get(
    ...
    if retrieval not in VALID_RETRIEVALS:
        raise ValueError(
            f"Unsupported selected neural retrieval mode: {retrieval!r}; "
            f"expected one of {VALID_RETRIEVALS}"
        )

    neighborhood_k = int(
        configuration.get(
            "neighborhood_k",
            configuration.get("csls_k", 10),
        )
    )
    if neighborhood_k < 1:
        raise ValueError(
            f"Selected neural neighborhood_k must be positive, got {neighborhood_k}"
        )
    return retrieval, neighborhood_k
...
def compute_neural_score_matrix(
    ...
    """Compute the selected neural criterion over the complete candidate pool."""
...
def load_or_build_neural_score_matrix(
    ...
    retrieval, neighborhood_k = neural_retrieval_settings(selected_neural)
    cache_dir.mkdir(parents=True, exist_ok=True)
    cache_path = cache_dir / (
        "neural_scores_"
        f"{neural_score_cache_key(embeddings_path, retrieval, neighborhood_k)}.npy"
    )
    if cache_path.is_file():
        matrix = np.load(cache_path, mmap_mode="r")
        if matrix.shape == (expected_rows, expected_rows):
    ...
    print(
        f"Computing full {retrieval} neural score matrix for {expected_rows} rows "
        f"with neighborhood_k={neighborhood_k}"
    )
    scores = compute_neural_score_matrix(
        source,
        target,
        retrieval,
        neighborhood_k,
...
def build_configuration(
    ...
    if candidate_generator not in {"neural_first", "ibm1_first"}:
        raise ValueError(f"Unknown candidate generator: {candidate_generator}")
    neural_retrieval, neural_neighborhood_k = neural_retrieval_settings(selected_neural)
    return {
        "candidate_generator": candidate_generator,
        "first_stage_family": (
            "xlmr_lora_projection" if candidate_generator == "neural_first" else "ibm1"
        ...
        "neural_neighborhood_k": int(neural_neighborhood_k),
        "candidate_topk": int(candidate_topk),
        "neural_score_mode": neural_score_mode,
        "normalization": normalization,
        "formula": formula,
    ...
    record.update(
        {
            ...
            "ibm_cache": str(ibm_cache_path),
            "neural_score_cache": str(neural_score_cache_path),
            "neural_retrieval": configuration["neural_retrieval"],
            "neural_neighborhood_k": int(configuration["neural_neighborhood_k"]),
            "num_queries": int(len(features.raw_sources)),
            "full_candidate_pool_size": int(len(features.raw_targets)),
            "candidate_pool_size": candidate_topk,
            "available_first_stage_topk": int(features.candidate_indices.shape[1]),
...
def tune_dev(args: argparse.Namespace) -> None:
    ...
    search_manifest = {
        ...
        "selected_ibm1_configuration_name": selected_ibm["configuration_name"],
        "selected_neural_configuration_name": selected_neural["configuration_name"],
        "selected_neural_retrieval": neural_retrieval_settings(selected_neural)[0],
        "selected_neural_neighborhood_k": neural_retrieval_settings(selected_neural)[1],
        "candidate_generators": generators,
        "candidate_topks": [int(value) for value in args.candidate_topks],
        "neural_score_modes": score_modes,
        "normalizations": list(args.normalizations),
